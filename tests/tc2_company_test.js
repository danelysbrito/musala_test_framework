...
fixture("Test Suite Company Tab")
    ...
    test("TC-02c Verify the correct Facebook URL loads", async t => {
        await t
            ...
            .expect(getPageURL()).contains(URLDATA.facebookUrl);
    });

    // test.only("TC-02d Verify if the Musala Soft profile appears in facebook page", async t => {
    //     await t
    //         .click(FooterComp.facebookLink)
    //         .expect(MusalaFacebookPage.profilePicture.visible).ok();
    // });

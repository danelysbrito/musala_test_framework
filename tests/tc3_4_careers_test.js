...
test("TC-03a Verify that ‘Join Us’ page is opened", async (t) => {
  await t.expect(getPageURL()).contains(URLDATA.careersUrl);
});
...
test("TC-03b Verify that 4 main sections are shown: General Description, Requirements, Responsibilities, What we offer", async (t) => {
  ...
  await t
    .expect(JoinUsPage.generalDescription.visible)
    .ok()
    .expect(JoinUsPage.requirements.visible)
    .ok()
    .expect(JoinUsPage.responsibilities.visible)
    .ok()
    .expect(JoinUsPage.whatWeOffer.visible)
    .ok();
});
...
test("TC-03d Verify shown error messages: The field is required", async (t) => {
  ...
  await t
    ...
    .expect(ApplyForPage.applyForTitle.visible)
    .ok()
    ...
    .typeText(
      ApplyForPage.linkedinProfileInput,
      APPLYDATA[0].linkedInProfileLink
    )
    .typeText(ApplyForPage.messageInput, APPLYDATA[0].message)
    .click(ApplyForPage.agreeCheckBox)
    .click(ApplyForPage.sendButton)
    .expect(
      ApplyForPage.getErrorMesageLocator(APPLYDATA[0].errorMessage).visible
    )
    .ok();
});
...
test("TC-03e Verify shown error messages: The e-mail address entered is invalid.", async (t) => {
  ...
  await t
    ...
    .typeText(
      ApplyForPage.linkedinProfileInput,
      APPLYDATA[1].linkedInProfileLink
    )
    .typeText(ApplyForPage.messageInput, APPLYDATA[1].message)
    .click(ApplyForPage.agreeCheckBox)
    .click(ApplyForPage.sendButton)
    .expect(
      ApplyForPage.getErrorMesageLocator(APPLYDATA[1].errorMessage).visible
    )
    .ok();
});

test("TC-04 Verify the list with available positions by city", async (t) => {
